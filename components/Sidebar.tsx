...
export default function Sidebar({ collapsed }: SidebarProps) {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-screen bg-muted/30 flex flex-col justify-between p-4 border-r transition-all duration-300 ${collapsed ? "w-20" : "w-60"}`}>
      {/* Branding */}
      <div className={`flex items-center mb-6 space-x-2 ${collapsed ? "justify-center" : ""}`}>
        <Shield className="h-10 w-10 text-blue-400 drop-shadow-[0_0_8px_rgba(30,64,175,0.8)]" />
        {!collapsed && <h1 className="text-2xl font-bold text-blue-400 drop-shadow-[0_0_12px_rgba(59,130,246,0.9)]">Sentinel Sovereign</h1>}
      </div>
        ...
        {navItems.map(item => {
          ...
          return (
            <div key={item.name}>
              <Button
                variant="ghost"
                className={`w-full justify-start bg-transparent transition-transform duration-200 ${isActive ? "scale-105" : ""} hover:scale-105`}
                onClick={() => handleNavigation(item.name, item.path)}
              >
                <item.icon
                  className={`mr-2 h-5 w-5 text-blue-400 drop-shadow-[0_0_8px_rgba(30,64,175,0.8)] ${isActive ? "animate-pulse" : ""}`}
                />
                {!collapsed && <span className={`text-gray-300 ${isActive ? "text-white drop-shadow-lg" : ""}`}>{item.name}</span>}
              </Button>
                  ...
                  {item.subItems!.map(sub => {
                    ...
                    return (
                      <Button
                        key={sub.name}
                        variant="ghost"
                        className={`w-full justify-start text-sm bg-transparent ${isSubActive ? "text-white" : "text-gray-400"} hover:text-white hover:scale-105 transition-transform duration-200`}
